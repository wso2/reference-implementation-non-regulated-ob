...
public class AccountService {
    ...
    private static final String NICKNAME_ACCOUNT_1 = "Bills";
    private static final String NICKNAME_ACCOUNT_2 = "Savings";
    ...
    public Response getOneAccount(@PathParam("AccountId") String accountId) {
        return wrap(envelope("Account", account(accountId, nickname(accountId)),
                "/accounts/" + accountId));
    }

    /**
     * Returns the nickname of a demo account. Both this endpoint and the accounts list read it
     * from here, so that asking for one account cannot report a different nickname from the one
     * the list gave for it.
     *
     * @param accountId account id being rendered
     * @return the nickname for that account
     */
    private static String nickname(String accountId) {
        return DEMO_ACCOUNT_2.equals(accountId) ? NICKNAME_ACCOUNT_2 : NICKNAME_ACCOUNT_1;
    }
